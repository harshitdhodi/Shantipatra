...
const StrappingPoints = () => {
                ...
                Cell: ({ row }) => (
                    <div className="flex gap-4">
                        <button className="text-blue-500 hover:text-blue-700 transition">
                            <Link to={`/product/edit-strapping-points/${row.original._id}`}>
                                <FaEdit />
                            </Link>
                        </button>
                        <button
    ...
    const fetchProductDetail = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/strappingPoint/getstrappingPoint`, { withCredentials: true });
    ...
    const deleteProductDetail = async (id) => {
        try {
            await axios.delete(`/api/strappingPoint/deletestrappingPoint?id=${id}`, { withCredentials: true });
            fetchProductDetail();
            notify();
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="p-4 overflow-x-auto">
            ...
            <div className="flex md:flex-row flex-col justify-between md:items-center mb-4">
                <h1 className="text-xl font-bold text-gray-700 font-serif uppercase">Strapping Points</h1>
                <div className="flex gap-2 md:flex-row flex-col md:mt-0 mt-4">
                    <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300" title="Add">
                        <Link to="/product/add-strapping-points"><FaPlus size={15} /></Link>
                    </button>
                </div>
            </div>
            <div className="mb-4">
...
export default StrappingPoints;
