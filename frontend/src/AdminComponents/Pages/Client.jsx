...
const Clients = () => {
  ...
  const filteredclients = useMemo(() => {
    return clients.filter((achievement) =>
...
/api/image/download/${firstImage}`} alt="Achievement" className="w-32 h-20 object-cover" /> : null;
        ...
        Cell: ({ row }) => (
          <div className="flex  gap-4">
            <button className="text-blue-500 hover:text-blue-700 transition">
              <Link to={`/client/editclients/${row.original._id}`}><FaEdit /></Link>
            </button>
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const clientsWithIds = response.data.map((achievement, index) => ({
        ...achievement,
        id: index + 1,
      }));
  ...
  const deleteAchievement = async (id) => {
    try {
      const response = await axios.delete(`/api/client/deleteClient?id=${id}`, { withCredentials: true });
    ...
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <Link to="/client/add-client"><FaPlus size={15} /></Link>
        </button>
      </div>
      <div className="mb-4">
        <input
      ...
      {loadings ? (
        ...
        <>
          {clients.length == 0 ? <div className="flex justify-center items-center"><iframe className="w-96 h-96" src="https://lottie.host/embed/1ce6d411-765d-4361-93ca-55d98fefb13b/AonqR3e5vB.json"></iframe></div> : <table className="w-full mt-4 border-collapse overflow-x-auto" {...getTableProps()}>
