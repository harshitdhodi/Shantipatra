...
const GalleryTable = () => {
  ...
  const filteredgallery = useMemo(() => {
    return gallery.filter((achievement) =>
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <span
            ...
            onClick={() => navigate(`/gallery/editgellery/${row.original._id}`)}
          >
            {row.original.title}
          </span>
        ),
...
/api/image/download/${firstImage}`} alt="Gallery" className="w-32 h-20 object-cover" /> : null;
        },
        disableSortBy: true,
      },
      {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/gallery/getAll`, { withCredentials: true });
      const galleryWithIds = response.data.map((achievement, index) => ({
        ...achievement,
        id: index + 1,
      }));
    ...
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <Link to="/gallery/addgallery"><FaPlus size={15} /></Link>
        </button>
      </div>
      <div className="mb-4">
        <input
      ...
      {loadings ? (
        ...
        <>
          {gallery.length == 0 ? <div className="flex justify-center items-center"><iframe className="w-96 h-96" src="https://lottie.host/embed/1ce6d411-765d-4361-93ca-55d98fefb13b/AonqR3e5vB.json"></iframe></div> : <table className="w-full mt-4 border-collapse overflow-x-auto" {...getTableProps()}>
